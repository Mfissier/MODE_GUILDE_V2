...
async function COFFRE_DELETE(message)
{
    ...
    for (let i = 0; i < data_guild.name_guild.length; i++)
    {
        if (data_guild.channel_finances[i] == id_channel_send)
        {
            if (args[1])
            {
                ...
                message.channel.send('**'+parseInt(args[1], 10) + ' kamas ont été enlevé du coffre de guilde !**');
                break;
            }
            break;
        }
...
exports.COFFRE_DELETE = COFFRE_DELETE;