...
async function BOUTIQUE_RANG_ESPION(message)
{
    cmd_user = message.member.user;
    id_channel = message.channel.id;
    let args = message.content.split(/ +/g);
    let name_rang = 'espion';
    if (args.length > 1)
    {
    ...
    if (index_guild == -1)
        ...
        for (let i = 0; i < data_user._user.length; i++)
        {
            ...
            {
                if (data_user._user[i].rang_power == 'ESPION')
                {
                    message.channel.send('🤖  **Vous avez déjà le rang espion !**')
                    return;
                }
                ...
                data_user._user[i].rang_power = 'ESPION';
                ...
                message.member.roles.add(message.guild.roles.cache.find(element => element.name == 'ESPION'));
                ...
                if (!channel_s)
                    return;
                channel_s.send('**Le membre <@!'+ data_user._user[i].id +'> est devenu un espion !**');
    ...
    Le membre <@!' + data_user._user[i].id + '> doit recevoir le rang espion en jeu !**');
                break;
            }
        }
}
exports.BOUTIQUE_RANG_ESPION = BOUTIQUE_RANG_ESPION;