...
async function BOUTIQUE_RANG_RECRUTEUR(message)
{
    cmd_user = message.member.user;
    id_channel = message.channel.id; 
    let args = message.content.split(/ +/g);
    let name_rang = 'RECRUTEUR';
    if (args.length > 1)
    {
    ...
    if (index_guild == -1)
        ...
        for (let i = 0; i < data_user._user.length; i++)
        {
            ...
            {
                if (data_user._user[i].rang_power == 'RECRUTEUR')
                {
                    message.channel.send('🤖  **Vous avez déjà le rang RECRUTEUR !**')
                    return;
                }
                ...
                data_user._user[i].rang_power = 'RECRUTEUR';
                ...
                message.member.roles.add(message.guild.roles.cache.find(element => element.name == 'RECRUTEUR'));
                ...
                if (!channel_s)
                    return;
                channel_s.send('**Le membre <@!'+ data_user._user[i].id +'> est devenu un RECRUTEUR !**');
    ...
    Le membre <@!' + data_user._user[i].id + '> doit recevoir le rang __RECRUTEUR__ en jeu !**');
                break;
            }
        }
}
exports.BOUTIQUE_RANG_RECRUTEUR = BOUTIQUE_RANG_RECRUTEUR;