...
const CHECK_CMD = require('./commands/CHECK_CMD.js');
const { fun_check_for_create_profil } = require("./data/data_recovery/fun_check_for_create_profil");
const { fun_check_emoji_enter_guild } = require("./commands/Tutoriel/check_emoji/fun_check_emoji_enter_guild");
const { fun_react_donjon } = require("./commands/cmd_user/fun_react/fun_react_valide_donjon");
const { fun_react_atq } = require("./commands/cmd_user/fun_react/fun_react_valide_atq");
const { fun_react_defense } = require("./commands/cmd_user/fun_react/fun_react_valide_def");
const { fun_react_valide_skin } = require("./commands/cmd_user/fun_react/fun_react_valide_skin");
const { fun_react_valide_braconnier } = require("./commands/cmd_user/fun_react/fun_react_valide_braconnier");
const { fun_check_penitent } = require("./events/fun_check_penitent");
const { fun_react_absent } = require("./events/Emoji_absent/fun_react_absent");
const { fun_debug_reload } = require("./events/fun_debug_reload");
...
const { fun_sleep } = require("./commands/Fun/fun_sleep");
const { fun_display_stats, fun_display_perco } = require("./stats/fun_display_stats");
client.on('ready', () => {
...
const init = async () => {
	client.login(config.token); // Log in to the discord api
};
init();
...
client.on("message", function(message){
    ...
    if (message.channel.type == 'dm')
        return;
	CHECK_CMD.CHECK_CMD(message, client);
	// Envoyer dans une fonction !
});
...
client.on("messageReactionAdd", function(messageReaction, user){
    fun_check_emoji_enter_guild(messageReaction, user);
    fun_react_donjon(messageReaction, user);
    fun_react_atq(messageReaction,user);
    fun_react_defense(messageReaction,user);
    fun_react_valide_skin(messageReaction,user);
    fun_react_valide_braconnier(messageReaction,user);
    fun_react_absent(messageReaction,user);
...
client.on("ready", function(){
	...
	console.log(`Bot has started, with ${client.users.size} users, in ${client.channels.size} channels of ${client.guilds.size} guilds.`); 
  	...
  	client.user.setActivity("DEMONTAGE DE TESTE");
    fun_display_perco(client);
    fun_display_stats(client);
    fun_check_penitent(client);
    fun_debug_reload(client);
    fun_register_data_to_bot();
    fun_guild_data_recovery();
    fun_register_users();
